...
public:
	...
	virtual void on_connect(const int errorcode, const int sockectnum) { std::cout << "server accept connect, error code: " << errorcode << std::endl; };
	...
	virtual void on_send(const unsigned char* data, const int len, const int sockectnum) { /*std::cout << "server send data: " << std::string((const char*)data, len) << std::endl;*/ };
